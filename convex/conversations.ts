...
export const createConversation = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const exsitingConversation = await ctx.db.query("conversations")
            .filter((q) => q.or(q.eq(q.field("participants"), args.participants), q.eq(q.field("participants"), args.participants.reverse()))).first();
        if (exsitingConversation) return exsitingConversation._id;
...
export const cickUser = mutation({
    args: {
        ...
        const conversationWithDetails = await Promise.all(
            myConversations.map(async (conversation) => {
                ...
                if (!conversation.isGroup) {
                    const otherUsersId = conversation.participants.find(id => id !== user._id);
                    const userProfile = await ctx.db
                        .query("users")
                        .filter(q => q.eq(q.field("_id"), otherUsersId)).take(1);
